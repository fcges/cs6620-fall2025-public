...
def parse_log_content(log_content):
    ...
    for row in reader:
        if not row or len(row) <= max(
                filter(None, [
                    col_audio, col_long_start, col_long_end, col_long_error,
                    col_short_error, col_short_start, col_short_end
                ])):
            continue
        ...
        try:
            long_start = float(
                row[col_long_start]) if col_long_start is not None else None
            long_end = float(
                row[col_long_end]) if col_long_end is not None else None
            short_start = float(
                row[col_short_start]
            ) if col_short_start is not None and row[col_short_start] else None
            short_end = float(
                row[col_short_end]
            ) if col_short_end is not None and row[col_short_end] else None
        ...
        segment = {
            ...
            long_end,
            'longFormError':
            row[col_long_error] if col_long_error is not None else '',
            'shortFormError':
            row[col_short_error] if col_short_error is not None else '',
            'shortFormStart':
            short_start,
            'shortFormEnd':
            short_end
...
def serve_audio_file(filename):
    ...
    elif current_directory and os.path.exists(
...
def index():
    ...
    """
    # Visible marker for automated deployments: version + timestamp
    return f'''
...
def upload_log():
    ...
    try:
        ...
        return jsonify({
            ...
            True,
            "message":
            f"Log file uploaded and parsed successfully. {len(parsed_transcription_data)} entries found."
        })
    except Exception as e:
        app.logger.error(f"Error parsing uploaded log file: {e}")
        return jsonify({
            "success": False,
            "message": f"Error parsing uploaded log file: {str(e)}"
        })
...
def load_log_from_path():
    ...
    if not os.path.exists(log_file_path):
        return jsonify({
            ...
            False,
            "message":
            f"Log file not found at path: {log_file_path}"
        })
    ...
    if not os.path.isfile(log_file_path):
        return jsonify({
            ...
            False,
            "message":
            f"Provided path is not a file: {log_file_path}"
        })
    ...
    try:
        ...
        return jsonify({
            ...
            True,
            "message":
            f"Log file loaded from path and parsed successfully. {len(parsed_transcription_data)} entries found."
        })
    except Exception as e:
        app.logger.error(
        ...
        return jsonify({
            ...
            False,
            "message":
            f"Error loading or parsing log file: {str(e)}"
        })
    ...
    try:
        ...
        for row in reader:
            ...
            if all(col in row for col in [
                    'recordErrorID', 'recordFile', 'exampleExample',
                    'recordTime'
            ]):
                # Skip rows where recordTime is empty or invalid
                try:
                    record_time = time_to_seconds(
                        row['recordTime']) if row['recordTime'].strip(
                        ) else 0.0
        ...
        return jsonify({
            "success": True,
            "message":
            f"CSV loaded successfully. {len(csv_error_data)} error records found.",
...
def save_label():
    ...
    try:
        ...
        if not all(key in data for key in
                   ['record_id', 'start_time', 'end_time', 'audio_file']):
            return jsonify({
                "success": False,
        ...
        with open(output_file, 'a', newline='', encoding='utf-8') as csvfile:
            fieldnames = [
                'record_id', 'audio_file', 'error_phrase', 'start_time',
                'end_time', 'duration', 'labeled_at'
            ]
        ...
        return jsonify({
            ...
            True,
            "message":
            f"Label saved for record {data['record_id']} to {output_file}"
        })
...
def download_labels():
    ...
    if os.path.exists(output_file):
        return send_file(output_file,
                         as_attachment=True,
                         download_name="labeled-segments.csv")
    else:
...
def view_labels():
    ...
    if not os.path.exists(output_file):
        return jsonify({
            "success": False,
            "message": "No labeled segments file found",
            "data": []
        })
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "message": f"Error reading labels file: {str(e)}",
            "data": []
        })
    ...
    try:
        if os.path.exists(output_file):
            ...
            return jsonify({
                ...
                True,
                "message":
                "All labeled segments deleted successfully"
            })
        else:
            return jsonify({
                "success":
                False,
                "message":
                "No labeled segments file found to delete"
            })
    except Exception as e:
        return jsonify({
            "success": False,
...
@app.route('/health')
def health():
    """Simple health endpoint exposing automated deployment info."""
    return {
        'status': 'healthy',
        'version': '2.0',
